...
### 1. What production problem does Azure Configuration and Secrets Management solve in a .NET application?

**Answer:** Azure configuration and secrets management separates environment-specific settings from code and protects sensitive values. The production problem is usually hosting, scaling, messaging, storage, secrets, telemetry, deployment, or reliability. A strong answer ties the service to one workload and one operational risk.
