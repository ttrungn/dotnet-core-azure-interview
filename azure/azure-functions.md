...
### 1. What production problem does Azure Functions solve in a .NET application?

**Answer:** Azure Functions runs event-driven code without managing servers, commonly for background tasks, integrations, and scheduled jobs. The production problem is usually hosting, scaling, messaging, storage, secrets, telemetry, deployment, or reliability. A strong answer ties the service to one workload and one operational risk.
...
### 7. How would you explain Azure Functions to a product owner without using unnecessary jargon?

**Answer:** I would explain Azure Functions as the Azure capability that helps the team run the system reliably, recover from failures, and support growth without manual server management.

**Example:** For example, a checkout API can use managed identity to read secrets from Key Vault, publish order events to Service Bus, and send telemetry to Application Insights.

### 8. How would you migrate an existing production feature toward better use of Azure Functions without stopping delivery?
