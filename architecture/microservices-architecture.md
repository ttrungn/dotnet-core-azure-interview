...
### 1. What is Microservices Architecture, and where have you used it in a .NET backend project?

**Answer:** Microservices split a system into independently deployable services aligned to business capabilities. The value is clearer ownership of rules and change. In an interview, explain the boundary it creates and the risk it reduces, not only the pattern name.
...
### 2. What problem does Microservices Architecture solve for an API or business application?

**Answer:** Microservices split a system into independently deployable services aligned to business capabilities. The value is clearer ownership of rules and change. In an interview, explain the boundary it creates and the risk it reduces, not only the pattern name.

**Example:** For example, an order controller calls `ConfirmOrderHandler`, the handler loads the order, the domain object enforces `Confirm()`, and infrastructure saves changes or sends messages.

### 3. How would you implement or apply Microservices Architecture in an ASP.NET Core service?

**Answer:** Microservices split a system into independently deployable services aligned to business capabilities. In practice, show who owns the business rules, how dependencies flow, how the design is tested, and when the added structure is worth the cost.

**Example:** For example, an order controller calls `ConfirmOrderHandler`, the handler loads the order, the domain object enforces `Confirm()`, and infrastructure saves changes or sends messages.

### 4. What are common mistakes developers make with Microservices Architecture?

**Answer:** Microservices split a system into independently deployable services aligned to business capabilities. In practice, show who owns the business rules, how dependencies flow, how the design is tested, and when the added structure is worth the cost.

**Example:** For example, an order controller calls `ConfirmOrderHandler`, the handler loads the order, the domain object enforces `Confirm()`, and infrastructure saves changes or sends messages.

### 5. What trade-offs should a senior developer consider before using Microservices Architecture?

**Answer:** Microservices split a system into independently deployable services aligned to business capabilities. In practice, show who owns the business rules, how dependencies flow, how the design is tested, and when the added structure is worth the cost.

**Example:** For example, an order controller calls `ConfirmOrderHandler`, the handler loads the order, the domain object enforces `Confirm()`, and infrastructure saves changes or sends messages.

### 6. An order API is slow, hard to test, and risky to deploy. How could Microservices Architecture help, and what would you check first?

**Answer:** Microservices split a system into independently deployable services aligned to business capabilities. In practice, show who owns the business rules, how dependencies flow, how the design is tested, and when the added structure is worth the cost.

**Example:** For example, an order controller calls `ConfirmOrderHandler`, the handler loads the order, the domain object enforces `Confirm()`, and infrastructure saves changes or sends messages.

### 7. How would you explain Microservices Architecture to a product owner without using unnecessary jargon?

**Answer:** I would explain Microservices Architecture as a way to reduce release risk by keeping important business behavior easier to change and test.

**Example:** For example, an order controller calls `ConfirmOrderHandler`, the handler loads the order, the domain object enforces `Confirm()`, and infrastructure saves changes or sends messages.

### 8. How would you migrate an existing production feature toward better use of Microservices Architecture without stopping delivery?
