...
### 1. What is Entities and Value Objects, and where have you used it in a .NET backend project?

**Answer:** Entities have identity over time, while value objects are defined by their values and usually immutable. The value is clearer ownership of rules and change. In an interview, explain the boundary it creates and the risk it reduces, not only the pattern name.
...
### 2. What problem does Entities and Value Objects solve for an API or business application?

**Answer:** Entities have identity over time, while value objects are defined by their values and usually immutable. The value is clearer ownership of rules and change. In an interview, explain the boundary it creates and the risk it reduces, not only the pattern name.

**Example:** For example, an order controller calls `ConfirmOrderHandler`, the handler loads the order, the domain object enforces `Confirm()`, and infrastructure saves changes or sends messages.

### 3. How would you implement or apply Entities and Value Objects in an ASP.NET Core service?

**Answer:** Entities have identity over time, while value objects are defined by their values and usually immutable. In practice, show who owns the business rules, how dependencies flow, how the design is tested, and when the added structure is worth the cost.

**Example:** For example, an order controller calls `ConfirmOrderHandler`, the handler loads the order, the domain object enforces `Confirm()`, and infrastructure saves changes or sends messages.

### 4. What are common mistakes developers make with Entities and Value Objects?

**Answer:** Entities have identity over time, while value objects are defined by their values and usually immutable. In practice, show who owns the business rules, how dependencies flow, how the design is tested, and when the added structure is worth the cost.

**Example:** For example, an order controller calls `ConfirmOrderHandler`, the handler loads the order, the domain object enforces `Confirm()`, and infrastructure saves changes or sends messages.

### 5. What trade-offs should a senior developer consider before using Entities and Value Objects?

**Answer:** Entities have identity over time, while value objects are defined by their values and usually immutable. In practice, show who owns the business rules, how dependencies flow, how the design is tested, and when the added structure is worth the cost.

**Example:** For example, an order controller calls `ConfirmOrderHandler`, the handler loads the order, the domain object enforces `Confirm()`, and infrastructure saves changes or sends messages.

### 6. An order API is slow, hard to test, and risky to deploy. How could Entities and Value Objects help, and what would you check first?
