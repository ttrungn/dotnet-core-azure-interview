...
Mocking replaces a dependency in a test so the test can focus on one behavior.

- **Mock:** Verifies interactions or configured behavior.
- **Fake:** Simple working implementation for tests.
- **Stub:** Returns predefined data.
- **Good use:** External services, clocks, email senders, payment gateways.
- **Bad use:** Mocking every internal class until the test only checks implementation details.

Example: mock `IPaymentGateway` to return a declined payment and verify the order is not confirmed.
...
### 1. What does Mocking improve in a .NET codebase?

**Answer:** Mocking replaces external collaborators in tests so behavior can be verified without calling real infrastructure. The value is fast feedback and lower regression risk. Explain what defect it catches and how it helps developers change code safely.
