...
Integration testing verifies that real components work together.

- **API integration:** Test HTTP request, routing, validation, auth, and response.
- **Database integration:** Test EF Core mapping, migrations, queries, and constraints.
- **Messaging integration:** Test publishing and consuming behavior where practical.
- **Realistic dependencies:** Use containers, test databases, or test hosts when behavior matters.
- **Slower but deeper:** Integration tests are fewer than unit tests but catch boundary issues.

Example: test `POST /orders` through the API and verify the order is saved in the test database.
...
### 1. What does Integration Testing improve in a .NET codebase?

**Answer:** Integration testing verifies real interactions between components such as APIs, databases, authentication, and messaging. The value is fast feedback and lower regression risk. Explain what defect it catches and how it helps developers change code safely.
...
### 7. How would you explain Integration Testing to a product owner without using unnecessary jargon?

**Answer:** I would explain Integration Testing as a way to catch defects earlier and reduce the chance that customers see broken behavior after release.

**Example:** For example, a discount calculation should have unit tests for edge cases, while order persistence should have an integration test against a real database provider.

### 8. How would you migrate an existing production feature toward better use of Integration Testing without stopping delivery?

**Answer:** Start with characterization tests around the riskiest existing behavior, then improve design and coverage as each touched area changes.
