...
Clean code communicates intent and keeps change cost low.

- **Names:** Classes, methods, and variables should reveal purpose.
- **Small functions:** Keep logic focused and readable.
- **Low duplication:** Remove duplication when it reduces real maintenance cost.
- **Clear boundaries:** Keep business rules out of controllers and infrastructure details.
- **Simple flow:** Prefer straightforward code over clever abstractions.

Example: `CalculateInvoiceTotal()` is clearer than `Process()` because the method name says what business result it produces.
...
### 1. What does Clean Code improve in a .NET codebase?

**Answer:** Clean code is code that communicates intent, keeps responsibilities focused, and avoids unnecessary complexity. The value is fast feedback and lower regression risk. Explain what defect it catches and how it helps developers change code safely.
